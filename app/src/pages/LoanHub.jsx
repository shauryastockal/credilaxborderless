...
export default function LoanHub() {
  ...
  return (
    <div className="p-4 space-y-5 max-w-2xl">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-gray-900">Loan Hub</h1>
        <p className="text-xs text-gray-400">Credila Education Loan</p>
      </div>

      {/* Loan summary card */}
      <div className="rounded-2xl p-5 border border-blue-100" style={{ background: 'linear-gradient(145deg, #eef4ff 0%, #e8f0fe 100%)' }}>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <h2 className="font-semibold text-gray-800 text-sm mb-4">Balance Status</h2>
        ...
        <div className="flex gap-3 mb-4">
          <div className="flex-1 bg-orange-50 rounded-xl p-3 border border-orange-100">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <p className="text-xs text-orange-600 font-medium">Lien (Locked)</p>
            </div>
            <p className="text-xl font-bold text-orange-600">${loan.lienBalance.toLocaleString()}</p>
            <p className="text-xs text-orange-400 mt-0.5">Held by Credila</p>
          </div>
          <div className="flex-1 bg-[#e8f0fe] rounded-xl p-3 border border-blue-100">
            <div className="flex items-center gap-1.5 mb-1">
        ...
        {showRequest && (
          <div className="mt-4 p-4 bg-[#f5f7fa] rounded-xl space-y-3">
            <p className="text-sm font-medium text-gray-700">Lien Release Request</p>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Amount to Release (USD)</label>
              <input
                type="number"
                placeholder="e.g. 1000"
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#0062db]"
              />
            </div>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Purpose</label>
              <select className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#0062db] bg-white">
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Upload Receipt / Invoice</label>
              <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center cursor-pointer hover:border-[#0062db] transition-colors">
                <Upload size={20} className="mx-auto text-gray-300 mb-1" />
                <p className="text-xs text-gray-400">Tap to upload PDF or image</p>
              </div>
            </div>
            {requested ? (
              <div className="flex items-center gap-2 bg-green-50 rounded-xl px-4 py-3">
                <CheckCircle size={16} className="text-green-500" />
                <p className="text-sm text-green-700 font-medium">Request submitted! Credila will review in 2–3 business days.</p>
              </div>
            ) : (
              <button
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold text-gray-800 text-sm">Next Tranche</h2>
          <span className="text-xs bg-blue-50 text-[#0062db] font-medium px-2 py-1 rounded-lg">Upcoming</span>
        </div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-2xl font-bold text-gray-900">${loan.nextTranche.toLocaleString()}</p>
            <p className="text-xs text-gray-400 mt-0.5">Expected {loan.nextTrancheDate}</p>
          </div>
        ...
        <div className="space-y-2">
          {loan.tranches.map(t => (
            <div key={t.id} className="flex items-center gap-3">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${t.status === 'disbursed' ? 'bg-green-100' : 'bg-gray-100'}`}>
                {t.status === 'disbursed'
              ...
              <div className="flex-1 flex justify-between">
                <p className="text-xs text-gray-700">Tranche {t.id} · {t.date}</p>
                <p className={`text-xs font-semibold ${t.status === 'disbursed' ? 'text-green-600' : 'text-gray-400'}`}>
      ...
      {/* Debt Breakdown */}
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <h2 className="font-semibold text-gray-800 text-sm mb-3">Debt Tracker</h2>
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="text-center">
            <p className="text-xs text-gray-400 mb-1">Total Debt</p>
            <p className="text-base font-bold text-gray-900">${loan.totalDebt.toLocaleString()}</p>
          </div>
          <div className="text-center border-x border-gray-100">
            <p className="text-xs text-gray-400 mb-1">Principal</p>
            <p className="text-base font-bold text-gray-900">${loan.principal.toLocaleString()}</p>
          </div>
          <div className="text-center">
        ...
        <div className="bg-[#f5f7fa] rounded-xl p-3 flex justify-between items-center">
          <div>
            <p className="text-xs text-gray-500">EMI Starts</p>
            <p className="text-sm font-semibold text-gray-800">{loan.emiStartDate}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-gray-500">Interest Rate</p>
            <p className="text-sm font-semibold text-gray-800">{loan.interestRate}% p.a.</p>
          </div>
        </div>
      </div>
      ...
      {/* Report Submission */}
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <h2 className="font-semibold text-gray-800 text-sm mb-1">Submit Academic Report</h2>
        <p className="text-xs text-gray-400 mb-3">Required by Credila for continued disbursement</p>
        <button
        ...
        {showUpload && (
          <div className="mt-3 p-4 bg-[#f5f7fa] rounded-xl space-y-3">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Document Type</label>
              <select className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-[#0062db]">
